...
GitTask.prototype.$removeRemote = function $removeRemote (callback) {
    function onRemove (err) {
        if (err) {
            debug('failed to remove remote repository. But it is not too bad. Means it does not exist');
        }
...
GitTask.prototype.$addRemote = function $addRemote (callback) {
    function onAdd (err) {
        if (err) {
            debug('failed to add remote repository');
            return callback(new VError(err, 'failed to execute `git remote add`'));
        }
...
GitTask.prototype.$push = function $push () {
    ...
    function onRemoveRemote (err) {
        if (err) {
            return ee.emit('error', new VError(err, 'failed to install the remote repository'));
        }
    ...
    function onGetBranch (err, branch) {
        ...
        shell.exec('git push ' + self.$remote + ' ' + branch + ':master', onPush);
    }
